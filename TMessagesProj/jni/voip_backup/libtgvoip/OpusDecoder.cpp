...
int tgvoip::OpusDecoder::DecodeNextFrame(){
	...
	int size;
	if(len){
		size=opus_decode(isEC ? ecDec : dec, buffer, len, (opus_int16 *) decodeBuffer, packetsPerFrame*480, fec ? 1 : 0);
		consecutiveLostPackets=0;
		if(prevWasEC!=isEC && size){
			...
			size=opus_decode(prevWasEC ? ecDec : dec, NULL, 0, (opus_int16*)nextBuffer, packetsPerFrame*480, 0);
			if(size){
		...
		if(consecutiveLostPackets>2 && enableDTX){
			...
			size=packetsPerFrame*480;
		}else{
			size=opus_decode(prevWasEC ? ecDec : dec, NULL, 0, (opus_int16 *) decodeBuffer, packetsPerFrame*480, 0);
			//LOGV("PLC");
		}
	}
	if(size<0)
...
void tgvoip::OpusDecoder::SetFrameDuration(uint32_t duration){
	...
	packetsPerFrame=frameDuration/10;
}
