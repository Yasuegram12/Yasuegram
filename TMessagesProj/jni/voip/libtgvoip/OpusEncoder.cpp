...
tgvoip::OpusEncoder::OpusEncoder(MediaStreamItf *source, bool needSecondary):queue(11), bufferPool(480*2, 10){
	this->source=source;
...
void tgvoip::OpusEncoder::RunThread(){
	...
	if(packetsPerFrame>1)
		frame=(int16_t*) malloc(480*2*packetsPerFrame);
	else
		frame=NULL;
	...
	while(running){
		...
		if(packet){
			bool hasVoice=true;
			if(echoCanceller)
				echoCanceller->ProcessInput(packet, 480, hasVoice);
			if(!postProcEffects.empty()){
				for(effects::AudioEffect* effect:postProcEffects){
					effect->Process(packet, 480);
				}
			}
			if(packetsPerFrame==1){
				Encode(packet, 480);
			}else{
				memcpy(frame+(480*bufferedCount), packet, 480*2);
				...
				if(bufferedCount==packetsPerFrame){
					...
					Encode(frame, 480*packetsPerFrame);
					bufferedCount=0;
					frameHasVoice=false;
				}
			}
